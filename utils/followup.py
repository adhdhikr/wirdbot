...
async def send_followup_message(guild_id: int, bot, session_id: int = None):
    ...
    try:
        ...
        for user in registered_users:
            ...
            if count == 0:
                not_started.append(f"__{member.display_name}__")
                ...
                if user_id in late_user_ids:
                    ...
                    late_completions_list.append(f"__{member.display_name}__")
                else:
                    ...
                    completed.append(f"__{member.display_name}__{streak}")
            else:
                # Still in progress (whether late or not)
                in_progress.append(f"__{member.display_name}__ - {count}/{total_pages} pages")
